...
import com.puputuan.generic.permission.dao.UserRoleRelativeDAO;
import com.puputuan.generic.permission.model.Role;
import com.puputuan.generic.permission.model.UserRoleRelative;
import com.puputuan.generic.permission.service.UserRoleRelativeService;
 ...
 */
@Service("userRoleRelativeService")
@Scope(value = ConfigurableBeanFactory.SCOPE_SINGLETON, proxyMode = ScopedProxyMode.INTERFACES)
public class UserRoleRelativeServiceImpl implements UserRoleRelativeService {


    @Resource(name = "userRoleRelativeDAO")
    private UserRoleRelativeDAO userRoleRelativeDAO;

    @Override
    public boolean add(UserRoleRelative object) {
        return userRoleRelativeDAO.insertSelective(object) > 0;
    }

    @Override
    public boolean remove(Long id) {
        return userRoleRelativeDAO.deleteByPrimaryKey(id) > 0;
    }

    @Override
    public boolean update(UserRoleRelative object) {
        return userRoleRelativeDAO.updateByPrimaryKeySelective(object) > 0;
    }

    @Override
    public UserRoleRelative getById(Long id) {
        return userRoleRelativeDAO.selectByPrimaryKey(id);
    }

    @Override
    public List<UserRoleRelative> getAll() {
        return userRoleRelativeDAO.selectParam(new HashMap());
    }
    ...
    public PagingResult<UserRoleRelative> getByPagination(PageEntity pageEntity) {
        ...
        List<UserRoleRelative> list = userRoleRelativeDAO.selectParam(pageEntity.getParam());
