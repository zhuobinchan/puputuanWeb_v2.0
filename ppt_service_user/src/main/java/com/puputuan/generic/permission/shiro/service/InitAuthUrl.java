package com.puputuan.generic.permission.shiro.service;
    ...
    public static StringBuffer initPermissionUrlFromProperties(StringBuffer stringBuffer) {
        ...
        while (it.hasNext()) {
            ...
            if (persMutil.length == 1) {
                permission = new StringBuffer(persMutil[0] + ":*");
            } else if (persMutil.length == 2) {
                permission =  new StringBuffer(persMutil[0] + ":" + persMutil[1] + ":*");
            } else {
                permission =  new StringBuffer(permissions.get(permissions.size() - 1).replaceAll("\\.", ":"));
            }
            ...
            for (int i = 0; i < permissions.size() - 1; i++) {
                ...
                if (persMutil.length == 1) {
                    permission.append("," + persMutil1[0] + ":*");
                } else if (persMutil.length == 2) {
                    permission.append("," + persMutil1[0] + ":" + persMutil1[1] + ":*");
                } else {
                    permission.append("," + permissions.get(i).replaceAll("\\.", ":"));
                }
            }
